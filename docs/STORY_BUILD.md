...
Story 5を読んだ人が「Sidekick Plannerとは何か」を探したときの**受け皿**であり、
**完成製品ページでも、開発状況の棚卸しページでもない。**

```
Storyを読む → Plannerに興味を持つ → 何をするソフトなのか分かる → 現在開発中だと分かる
```

この4段階で足りる。それ以上の情報を足さない。

### 2026-08-31 公開前レビューでの村田さんの決定

初版は Planner リポジトリの `CURRENT_WORK.md`・`OPEN_QUESTIONS.md` を実測して
「検証用プロトタイプで動いているもの」「Version 1で予定している範囲」
「まだ決まっていないこと」「Version 1では対象にしないこと」の**4枚のカード**に分けて
掲載していた。村田さんのレビューで**公開する開発内部情報が詳しすぎる**と判断され、
「現在開発している主な機能」1枚（4項目）へ抽象化した。

**以下は意図的に公開しない。AI判断で復活させないこと。**

| 載せないもの | 理由 |
|---|---|
| 現在どこまで実装済みか（プロトタイプの到達状況） | 競合に対して開発状況を先に開示することになる |
| 「ダイヤモンド／パール」という機能名 | Diamond XX展開の露出を避ける（`project_diamond_xx_expansion_strategy`は壁打ち段階・実装保留）。**Story 5本文にパール富士が登場すること自体は問題ない** |
| 3D都市モデル・PLATEAU・標高データ等の具体的なデータソース名 | 正式な製品仕様・attributionが必要になった時点で扱う |
| Version 1 対象外の列挙（Web/スマートフォン版・天の川アーチ構図・雲量予報・アプリ内共有機能） | 予告ページとして読者に必要性が低い。**とくに共有機能はQR Shareトラックの現在の状態と整合が取れない** |
| 内部Product Scope（外部AI APIを使わない／text・CSV出力／共有をOSに任せる／データ配布方式／更新方式／認証方式／コード署名） | 同上 |
| 価格・提供時期・提供形態 | 未決。「決まり次第お知らせします」に留める |

**以下は残すと決まっている。**

- 「写真の正解を出すソフトではありません」
- 計算できることはコンピューターに任せる
- 写真家が現場で判断するための候補を見つける
- 計算で分かることと、現場でしか分からないことがある
- 「ここへ行けばいい写真が撮れる」と保証しない
- Story 5 への導線
- 現在開発中であること
- メーカー等がPlannerを推薦・公認しているわけではない旨

### 実装上の決まりごと

- 「現在開発中 / In development」を**バッジ・冒頭の注意書き・末尾の但し書きの3箇所**で
  明示する（`tests/story/test_ja_en_parity.py::test_planner_page_states_it_is_in_development`
  で検証）
- 掲載する機能は、読者にとっての**できること**の粒度で書く。実装状況・内部方式・
  データソース名を混ぜない
- 記載内容の根拠は`Sidekickシリーズ/本体/Sidekick Planner/`の`README.md`・
  `CURRENT_WORK.md`・`docs/OPEN_QUESTIONS.md`を**実測して確認したものだけ**。
  会話の記憶から機能を列挙しない。**そのうえで、実測できたからといって
  公開してよいとは限らない**（上表のとおり）
- テンプレート冒頭のコメントに、この決定の全文を書いてある。テンプレートを編集する人が
  このドキュメントを読んでいなくても分かるようにするため
