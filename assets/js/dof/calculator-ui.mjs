...
const $=id=>document.getElementById(id);const form=$("dof-form");let history=null;let draftChanged=false;
...
function calculate(){try{clearErrors();const value=input(),result=calculateClassicDof(value);history=advanceComparison(history,record(value,result));draftChanged=false;renderCurrent();updateCriterionUx();updateInputComparison();if($("dof-curve-disclosure").open)draw();}catch(error){showError(error);updateInputComparison();}}
...
form.addEventListener("submit",e=>{e.preventDefault();calculate();});form.addEventListener("input",noteDraftChange);form.addEventListener("change",e=>{if(e.target===$("dof-sensor")||e.target===$("dof-criterion"))toggleOptional();noteDraftChange();});
for(const id of ["dof-sensor-width","dof-sensor-height","dof-criterion-value"])$(id).addEventListener("input",updateCriterionUx);
for(const button of document.querySelectorAll("[data-fnumber]"))button.addEventListener("click",()=>{$("dof-aperture").value=button.dataset.fnumber;noteDraftChange();$("dof-aperture").focus();});
