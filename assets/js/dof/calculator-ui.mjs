...
function calculate(){try{clearErrors();const value=input(),result=calculateClassicDof(value);if(history?.current&&changedInputKeys(history.current.inputSnapshot,inputSnapshot()).length===0){draftChanged=false;updateInputComparison();return;}history=advanceComparison(history,record(value,result));draftChanged=false;renderCurrent();updateCriterionUx();updateInputComparison();if($("dof-curve-disclosure").open)draw();}catch(error){showError(error);updateInputComparison();}}
...
form.addEventListener("submit",e=>{e.preventDefault();});form.addEventListener("input",noteDraftChange);form.addEventListener("change",e=>{if(e.target===$("dof-sensor")||e.target===$("dof-criterion"))toggleOptional();noteDraftChange();calculate();});
